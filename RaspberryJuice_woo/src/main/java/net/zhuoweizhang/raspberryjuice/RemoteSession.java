...
public class RemoteSession {
	...
	protected void handleCommand(String c, String[] args) {
		...
		try {
			...
			if (c.equals("world.getBlock")) {
				...
				// String chatMessageTest = "";
				// int count;
				// for (count = 0; count < args.length; count++) {
				// 	chatMessageTest = chatMessageTest + args[count] + ",";
				// }
				// System.out.println(chatMessageTest);

				// String[] chatMessage = args;
				String chatMessage = "";
				// System.out.println("args: "+ Arrays.toString(args));
				// System.out.println("length of args:" + args.length);

				byte[] messageInDecimals = new byte[256];
				// chatMessage = chatMessage.substring(0, chatMessage.length - 1);
				for (int i = 0; i < args.length - 32; i++) {
					messageInDecimals[i] = (byte) Integer.parseInt(args[i]);
				}
				// System.out.println("message: " + Arrays.toString(messageInDecimals));
				// System.out.println("length of message:" + messageInDecimals.length);

				byte[] signatureInDecimals = new byte[32];
				// chatMessage = chatMessage.substring(0, chatMessage.length - 1);
				for (int i = 256; i < args.length; i++) {
					signatureInDecimals[i-256] = (byte) Integer.parseInt(args[i]);
				}
				// System.out.println("sig: " + Arrays.toString(signatureInDecimals));
				// System.out.println("length of sig:" + signatureInDecimals.length);

				// String macRes = hashAndConvertToBase64String(messageInDecimals);
				byte[] macRes = getMac(messageInDecimals);
				// System.out.println("mac computed on message: " + Arrays.toString(macRes));
				
				// If statement for checking if the mac is valid
				if (Arrays.equals(macRes, signatureInDecimals)) {
					System.out.println("MAC is VALID");
					chatMessage = decryptMessage(messageInDecimals);
				} else {
					System.out.println("MAC is INVALID!!!");
					chatMessage = "Text has been altered!";
				}
	...
	public static byte[] getMac(byte[] text) throws InvalidKeyException, Exception {
		Mac mac = Mac.getInstance("HmacSHA256");
		byte[] secret = readPrivateKey();
		System.out.println("getMac: secret = " + Arrays.toString(secret));
		SecretKeySpec secretKeySpec = new SecretKeySpec(secret, "HmacSHA256");
		System.out.println("getMac: secretkeyspec = " + Arrays.toString(secretKeySpec.getEncoded()));
		mac.init(secretKeySpec);
		byte[] macResult = mac.doFinal(text);
		return macResult;
	}
	// public static String hashAndConvertToBase64String(byte[] message) throws Exception {
    //     byte[] hash = getMac(message);
	...
	public static byte[] readPrivateKey() throws Exception {
		File file = new File("C:/Users/Joshua/github-classroom/rmit-computing-technologies/cosc2804-sep-22-assignment-3-team-40-cosc2804-sep22/RaspberryJuice_woo/src/main/resources/mcpi/api/python/modded/mcpi/privateKey.pem");
	...
	public Key getPKCS8key() throws Exception {
		byte[] encodedSecret = readPrivateKey();
		KeyFactory keyFactory = KeyFactory.getInstance("RSA");
		PKCS8EncodedKeySpec keySpec = new PKCS8EncodedKeySpec(encodedSecret);
		return (Key) keyFactory.generatePrivate(keySpec);
	}

	public String decryptMessage(byte[] message) throws Exception {
		Key privKey = getPKCS8key();
		Cipher decryptionCipher = Cipher.getInstance("RSA/ECB/OAEPPadding");
        decryptionCipher.init(Cipher.DECRYPT_MODE,privKey);
